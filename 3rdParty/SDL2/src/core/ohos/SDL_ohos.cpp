...
{
    size_t argc = 3;
    napi_value args[OHOS_INDEX_ARG3];
    ...
    len = 0;
    napi_get_value_string_utf8(env, args[1], g_assetPath, 0, &len);
    
    if (g_assetPath != nullptr) {
        delete g_assetPath;
        g_assetPath = nullptr;
    }
    
    g_assetPath = new char[len + 1];
    napi_get_value_string_utf8(env, args[1], g_assetPath, len + 1, &len);
...
napi_value SDLNapi::OHOS_NativeSetScreenResolution(napi_env env, napi_callback_info info)
{
    ...
    napi_get_value_double(env, args[OHOS_INDEX_ARG4], &screenDensity);
    napi_get_value_double(env, args[OHOS_INDEX_ARG5], &rate);
    SDL_LockMutex(g_ohosPageMutex);
    OHOS_SetScreenResolution(deviceWidth, deviceHeight, SDL_PIXELFORMAT_UNKNOWN, rate, screenDensity);
    SDL_UnlockMutex(g_ohosPageMutex);
    return nullptr;
}
