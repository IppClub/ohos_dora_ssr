...
export function setScreenResolution(){
  ...
  try {
    ...
    let densityPixels:number = mDisplay.densityPixels
    let rate:number = mDisplay.refreshRate

    sdl.nativeSetScreenResolution(nDeviceWidth,nDeviceHeight,nsurfaceWidth,nsurfaceHeight,densityPixels,rate)
  }
  catch (exception) {
...
export function setResourceManager(manager: Context) {
  sdl.setResourceManager(manager.cacheDir, manager.bundleCodeDir, manager.resourceManager)
}
